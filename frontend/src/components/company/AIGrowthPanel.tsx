...
export function AIGrowthPanel({ company, onClose }: Props) {
  ...
  if (riskOpen && analysis) return <RiskAnalysisPanel company={company} analysis={analysis} onBack={() => setRiskOpen(false)} onContinue={() => { setRiskOpen(false); setPlanNotice(true); }} />;
  return <aside className="theme-surface pointer-events-auto absolute inset-x-4 top-20 bottom-4 z-30 max-h-none overflow-y-auto rounded-lg border border-sky-200/15 bg-slate-950/95 p-4 shadow-[0_20px_60px_rgba(0,0,0,0.5)] backdrop-blur-md lg:inset-x-auto lg:right-5 lg:top-24 lg:bottom-auto lg:max-h-[calc(100%-7rem)] lg:w-[min(25rem,calc(100%-2.5rem))] lg:p-5 xl:right-8">

...
const cache = new Map<string, GrowthAnalysis>();

export function AIGrowthPanel({ company, onClose }: Props) {
  const [analysis, setAnalysis] = useState<GrowthAnalysis | null>(() => cache.get(company.id) ?? null);
  ...
  const load = async () => {
    ...
    try {
      ...
      cache.set(company.id, body.analysis); setAnalysis(body.analysis);
  ...
  return <aside className="pointer-events-auto absolute right-5 top-24 z-20 max-h-[calc(100%-7rem)] w-[min(25rem,calc(100%-2.5rem))] overflow-y-auto rounded-lg border border-sky-200/15 bg-slate-950/95 p-5 shadow-[0_20px_60px_rgba(0,0,0,0.5)] backdrop-blur-md sm:right-8">
    <div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-medium tracking-[0.18em] text-sky-300">AI GROWTH ANALYSIS</p><p className="mt-2 text-lg font-semibold text-white">{company.name}</p><p className="mt-1 text-xs text-sky-200">{[companyText(company.category), companyIndustry(company), company.city].filter(Boolean).join(" · ")}</p></div><button type="button" aria-label="Close AI growth analysis" onClick={onClose} className="text-slate-400 hover:text-white">✕</button></div>

...
export default function CompanyDetailsPanel({ company, onClose }: { company: Company | null; onClose: () => void }) {
  ...
  const businessModel = companyText(company.businessModel);
  ...
  return <div className="fixed inset-0 z-50 flex justify-end bg-slate-950/50 p-3 backdrop-blur-sm sm:p-5" role="dialog" aria-modal="true" aria-label={`${company.name} details`}>
    <section className="theme-surface flex h-full w-full max-w-md flex-col overflow-y-auto rounded-lg border p-5 shadow-2xl">
      ...
      <div className="mt-6 grid grid-cols-2 gap-3"><Detail label="Funding" value={formatFunding(company)} /><Detail label="Location" value={company.city || "Not available"} /><Detail label="Business model" value={businessModel || "Insufficient data available."} /><Detail label="Data confidence" value={[company.city, description, companySector(company)].filter(Boolean).length >= 3 ? "Medium" : "Low"} /></div>
      <div className="mt-6"><p className="text-xs uppercase tracking-[0.14em] text-slate-400">Description</p><p className="mt-2 text-sm leading-6 text-slate-200">{description || "Insufficient data available."}</p></div>
      ...
      {website ? <a href={website.startsWith("http") ? website : `https://${website}`} target="_blank" rel="noreferrer" className="mt-6 inline-flex w-fit rounded border border-sky-400/40 bg-sky-500/10 px-3 py-2 text-sm text-sky-100 hover:bg-sky-500/20">Visit website</a> : null}
    </section>
  </div>;
}

...
import { companySector, fundingAmount, investorNames } from "@/lib/investor";
...
type Brief = { summary: string; signals: { type: string; title: string; evidence: string; confidence: string }[]; researchQuestions: string[]; note: string; demo?: boolean };
...
export default function InvestorRadarDashboard({ companies }: { companies: Company[] }) {
  ...
  const filtered = useMemo(() => companies.filter((company) => {
    ...
    if (investorQuery.trim() && !investorNames(company).some((investor) => investor.toLowerCase().includes(investorQuery.trim().toLowerCase()))) return false;
    if (query.trim()) {
  ...
  return <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-4">
    ...
    <CompanyDetailsPanel company={selectedCompany} onClose={() => setSelectedCompany(null)} /><InvestorProfile investorName={selectedInvestor} onClose={() => setSelectedInvestor(null)} companies={companies} />{compareIds.length >= 2 ? <ComparePanel companies={companies.filter((company) => compareIds.includes(company.id))} onClose={() => setCompareIds([])} /> : null}
  </div>;
}

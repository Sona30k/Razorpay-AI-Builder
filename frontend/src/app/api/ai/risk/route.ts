...
function canonicalCategory(value: unknown) {
  ...
  return riskCategories.find((category) => {
    const expected = category.toLowerCase().replace(/[^a-z]/g, "");
    return normalized === expected || normalized === expected.replace(/risk$/, "");
  }) ?? null;
}
...
function normalizedAnalysis(value: unknown): RiskAnalysis | null {
  ...
  const risks = data.risks.map((item) => {
    ...
    const category = canonicalCategory(risk.category);
    if (!category || !level || !Number.isFinite(score) || score < 0 || score > 100 || typeof risk.reason !== "string" || typeof risk.mitigation !== "string") return null;
    return { category, level, score: Math.round(score), reason: risk.reason, mitigation: risk.mitigation };
  });
